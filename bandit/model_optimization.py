...
import numpy as np
from bandit.model_selection import EVALUATION_CRITERIA
...
class RacosOptimization(Optimization):
    ...
    def __init__(self, model_generator, name=None, b1=1, b2=1, c1=1, c2=1, alpha=1.0 / 3, theta=0.01,
    ...
    def run_one_step(self, train_x, train_y, total_count):
        start = time.time()
        ...
        # update parameters
        self.count += 1
        self._update_parameters(total_count)

    def selection_value(self, t):
        return self.beta1 * t + self.beta0 + self.e_beta1 * t + self.e_beta0 + np.sqrt(
            1 / self.theta * self.variance + self.e_variance)

    def _update_parameters(self, total_count):
        ...
        # update exploration item
        self._update_e_beta0(total_count)
        self._update_e_beta1(total_count)
        self._update_e_variance(total_count)
    ...
    def _update_variance(self, y_hat):
        ...
        self.variance = ((y - y_hat) ** 2).sum() / self.count

    def _update_e_beta1(self, t):
        numerator1 = stats.norm.ppf(self.c1 / t ** self.alpha - self._func_m(self.count))
        self.e_beta1 = - numerator1 * np.sqrt(self.variance) / np.sqrt(_func_a(self.count))

    def _update_e_beta0(self, t):
        x_hat = self._x_hat()
        item1 = stats.norm.ppf(self.c2 / t ** self.alpha - self._func_n(self.count))
    ...
    def _update_e_variance(self, t):
        self.e_variance = t * self.e_beta1 + self.e_beta0 + np.sqrt(2 * np.log(t) / self.count)
    ...
    def _func_m(self, n):
        ...
        item2 = abs(seq_i - x_hat) ** 3 * (abs(np.asarray(self.inverse_eval) - (seq_i * self.beta1 + self.beta0)) ** 3)
